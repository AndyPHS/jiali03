<template>
  <div class="article_all">
    ...
    <div v-if="status ==9" class="flex w-1/3 float-left">
      <span class="px-2 py-2 text-sm" @click="goBackList">返回文章列表</span>
    </div>
    <div class="flex w-1/3 float-right">
      <el-button class="mr-3" type="primary" size="mini" plain>上传新文章</el-button>
        ...
        <el-table :data="tableData" stripe height="500" :header-row-style="{height:'40px'}" :row-style="{height:'40px'}">
          ...
          <el-table-column label="操作" :width="status==1? '300':'150' ">
            <template slot-scope="scope">
              <el-button v-if="status ==1" type="primary" size="mini" round>编辑文章</el-button>
...
import {deleteNews} from '@/api/api/requestLogin.js' // 删除文章
import {recoveryNews} from '@/api/api/requestLogin.js' // 恢复文章
import {selectNews} from '@/api/api/requestLogin.js'  // 查询文章
import {selectFaIDNews} from '@/api/api/requestLogin.js'  // 通过分类查询文章
 export default{
   name: 'Article',
   data() {
     return {
