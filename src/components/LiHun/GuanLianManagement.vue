  ...
  <div>
      ...
      <div class="w-1/2">
        <div class="border border-1 rounded">
          ...
          <div class="h-40 overflow-scroll">
            ...
            <el-tree :data="treeList" default-expand-all node-key="id" ref="tree" empty-text="加载中，请稍后！"
              highlight-current @node-click="handleTreeJieDian()" :props="defaultProps" :draggable='true' @node-drop="handleDrop">
      ...
      <div class="w-1/2 mx-2">
        ...
        <div class="text-center question_list">
          <table class="w-full">
            ...
            <tbody>
              <tr v-for="(item, index) in pageInfo" :key="index">
        ...
        <el-pagination background class="my-10" layout="prev, pager, next" @current-change="handleQuestionList"
          :page-size="pagesize" :current-page.sync="currentPage" :total="this.min.total">
        </el-pagination>
      </div>
    </div>
    ...
    <el-dialog title="新增问题" :visible.sync="dialogQuestionAdd">
      <el-form :model="user">
        ...
        <div v-for="(item, index) in add_answer" :key="index" v-if="user.type==6 || user.type==7 || user.type==8 || user.type==9 "
    ...
    <el-dialog title="问题配置" :visible.sync="dialogQuestionConfig">
      <el-form :model="problemqAdd">
        ...
        <div v-if="problemqAdd.type==6 || problemqAdd.type==7 || problemqAdd.type==8 || problemqAdd.type==9 ">
          ...
          <div>
            ...
            <el-form-item label="问题" :label-width="formLabelWidth">
              <el-select v-model="whereItemConfig.relationId" filterable :filter-method="select_vague" placeholder="请选择">
                <el-option v-for="item in selectVagueValueArr" :key="item.id" :label="item.qpTitle" :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="条件" :label-width="formLabelWidth">
              <el-select v-model="whereItemConfig.type" placeholder="请选择">
                <el-option v-for="item in TiaoJianList" :key="item.value" :label="item.title" :value="item.value">
                </el-option>
              </el-select>
            </el-form-item>
            <div class="my-2">
          ...
          <div>
            <table class="w-full">
              <thead>
                <tr>
                  <td class="headstyle w-1/6 text-center border text-black font-bold py-2" v-for="item of whereThead"
                    :key="item.index">{{item}}</td>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(item, index) in selectQpWhereList" :key="index">
                  <td class="border text-black hover:cursor-pointer">{{ item.title }}</td>
        ...
        <!-- 选项关联其他问题 -->
        <div class="mt-3" v-if="problemqAdd.problemType ==6|| problemqAdd.problemType ==7 || problemqAdd.problemType ==8 || problemqAdd.problemType ==9">
          <div class="relative wenti">
            <span class="h-1"></span>
            <span class="px-2 py-2 border absolute left-0 top-0 z-10 bg-white">选项关联其他问题</span>
          </div>
          <div class="clear-both"></div>
          <div>
            <el-form-item label="绑定的值" :label-width="formLabelWidth">
              <el-select v-model="Pof.value" placeholder="选择绑定的值">
                <el-option v-for="(item, index) in selectOnlyLisg" :key="index" :label="item.label" :value="item.value"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="问题" :label-width="formLabelWidth">
            ...
            <el-form-item label="条件" :label-width="formLabelWidth">
              <el-select v-model="Pof.type" placeholder="请选择">
                <el-option v-for="item in TiaoJianList" :key="item.value" :label="item.title" :value="item.value">
                </el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="问题的值" :label-width="formLabelWidth">
              <el-select v-model="Pof.ofvalue" placeholder="选择绑定的值">
                <el-option v-for="(item, index) in selectPropList" :key="index" :label="item.label" :value="item.value"></el-option>
              </el-select>
            </el-form-item>
            <div class="my-2">
              <el-button @click="addPof">添加关联此问题</el-button>
              <!-- <el-button @click="deleteWhere(whereIndex)">删除关系</el-button> -->
            </div>
          </div>
          <div>
            <table class="w-full">
              <thead>
                <tr>
                  <td class="headstyle w-1/6 text-center border text-black font-bold py-2" v-for="item of PofThead"
                    :key="item.index">{{item}}</td>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(item, index) in selectPofPList" :key="index">
                  ...
                  <td class="border text-black justify-around">
                    <div class="flex justify-around py-2 w-2/3 mx-auto">
                      <!-- <el-button type="primary" plain class="" @click="dbclickQuestion(item.id)">修改</el-button> -->
                      <el-button type="primary" plain class="" @click="deleteprop(index)">删除</el-button>
                    </div>
                  </td>
                </tr>
              </tbody>
    ...
    <el-dialog title="修改配置" :visible.sync="dialogQuestionConfigUpdate">
      <el-form :model="problemqAdd">
        ...
        <el-form-item label="绑定问题" :label-width="formLabelWidth">
          <el-select v-model="problemqAdd.problemId" filterable :filter-method="dataFilter" @change="bangdingQuestion(problemqAdd.problemId)"
            placeholder="请选择">
            <el-option v-for="item in dataFilterValueArr" :key="item.id" :label="item.title" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
...
<script>
  ...
  import {
    addQuestion
  } from '@/api/api/requestLogin.js' // 新增问题
  import {
    addAnswer
  } from '@/api/api/requestLogin.js' // 添加选项
  import {
    selectQuestion
  } from '@/api/api/requestLogin.js' // 获取用户列表
  import {
    selectOnlyQuestion
  } from '@/api/api/requestLogin.js' // 查询单独问题
  import {
    selectQuestionList
  } from '@/api/api/requestLogin.js' // 根据标题条件获取用户列表
  import {
    updateQuestion
  } from '@/api/api/requestLogin.js' // 修改问题
  import {
    deleteQuestion
  } from '@/api/api/requestLogin.js' // 删除问题
  import {
    QuestionArr
  } from '@/api/api/requestLogin.js' // 问题数组
  import {
    selectTree
  } from '@/api/api/requestLogin.js' // 查询关系
  import {
    selectVague
  } from '@/api/api/requestLogin.js' // 模糊查询关系
  import {
    ProblemQAdd
  } from '@/api/api/requestLogin.js' // 新增关联
  import {
    addQpWhere
  } from '@/api/api/requestLogin.js' // 新增关联条件
  import {
    selectQpWhere
  } from '@/api/api/requestLogin.js' // 查询关联条件
  import {
    deleteQpWhere
  } from '@/api/api/requestLogin.js' // 删除关联条件
  import {
    deleteProblemQ
  } from '@/api/api/requestLogin.js' // 删除关联
  import {
    updateProblemQ
  } from '@/api/api/requestLogin.js' // 修改关联
  import {
    updateOrder
  } from '@/api/api/requestLogin.js' // 修改层级 父类
  ...
  export default {
    ...
    data() {
      return {
        ...
        selectPofPFilterArr: [], // 点击修改弹窗问题模糊搜索
        selectPofPValueArr: [], // 点击修改弹窗问题模糊搜索结果
        selectPofPList: [], // 点击修改的绑定问题选项关联的其他问题
        // whereArr: [{
        //     relationId: null,
        //     type: '',
        //     value: null
    ...
    methods: {
      ...
      addPof () {
        console.log(this.selectPofPList)
        if (this.selectPofPList==null) {
          this.selectPofPList = []
          this.selectPofPList.push(this.Pof)
        } else {
          this.selectPofPList.push(this.Pof)
        }
        // this.selectPofPList.push(this.Pof)
      ...
      selectTree() { // 查询树结构
        selectTree().then((data) => {
          ...
          // const tt = this.treeList
          // for (var i = 0; i < tt.length; i++) {
          //   const ee = tt[i].child
          //   for (var ii = 0; ii < ee.length; ii++) {
          //     const ff = ee[ii].child
          //     for (var oo = 0; oo < ff.length; oo++) {
          //       if(ff[oo].introduce == 0) {
          //         console.log(ff[oo].title)
          //       }
          //       const g = ff[oo].child
          //       for (var m = 0; m< g.length; m++) {
          //         if( g[m].introduce ==0) {
          //           console.log(g[m].title)
          //         }
          //         const gg = g[m].child
          //         for(var mm = 0;mm<gg.length; mm++) {
          //           if(gg[mm].introduce==0 ){
          //             console.log(gg[mm].title)
          //           }
          //         }
          //       }
          //     }
      ...
      handleTreeJieDian() {
        ...
        this.selectPofPList = JSON.parse(this.$refs.tree.currentNode.node.data.PofP)
      },
      ...
      addQuestionConfig() { // 点击确定提交当前问题的配置
        if (this.problemqAdd.type == 6 || this.problemqAdd.type == 7 || this.problemqAdd.type == 8 || this.problemqAdd.type ==
          9) {
          ProblemQAdd({
            questionnaireId: localStorage.getItem('qid'),
            problemId: this.problemqAdd.problemId,
            ...
            description: this.problemqAdd.description,
             PofP: JSON.stringify(this.selectPofPList)
          }).then((data) => {
            this.problemqAdd.title = '';
            this.problemqAdd.class = '';
            this.problemqAdd.type = '';
